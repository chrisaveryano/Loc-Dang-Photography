...
},{}],"QAWR":[function(require,module,exports) {
"use strict";
...
},{"./Observer.js":"QAWR"}],"QvaY":[function(require,module,exports) {
"use strict";
...
var allImages = document.querySelectorAll('.gallery__item-imginner');
var flowImages = document.querySelectorAll('.gallery__item-img--flow'); // Previous intersection ratio

var prevRatio = 0.0; // Slide-in animation

var obsFlowCallback = function obsFlowCallback(entries, _) {
  entries.forEach(function (entry) {
    if (entry.isIntersecting) {
      entry.target.firstElementChild.classList.add('flow');
    }

    if (!entry.isIntersecting) entry.target.firstElementChild.classList.remove('flow');
  });
}; // Image transition animation


var obsCallback = function obsCallback(entries, _) {
  entries.forEach(function (entry) {
    // Limit brightness and saturation at 100%
    var intersectVal = (0, _utils.lerp)(entry.intersectionRatio * 1.5, prevRatio, 0.1);
    intersectVal = intersectVal >= 1 ? 1 : intersectVal; // Animation

    if (entry.intersectionRatio > prevRatio) {
      entry.target.style.opacity = intersectVal;
      entry.target.style.filter = "saturate(".concat(intersectVal, ") brightness(").concat(intersectVal, ")");
    } else {
      entry.target.style.opacity = intersectVal;
      entry.target.style.filter = "saturate(".concat(intersectVal, ") brightness(").concat(intersectVal, ")");
    } // Make it smooth
...
var obs = new IntersectionObserver(obsCallback, obsOptions);
var obsFlow = new IntersectionObserver(obsFlowCallback, obsOptions);
allImages.forEach(function (img) {
  return obs.observe(img);
});
flowImages.forEach(function (img) {
  return obsFlow.observe(img);
}); // Register ScrollTrigger for horizontal scroll

_gsap.gsap.registerPlugin(_ScrollTrigger.ScrollTrigger);

var horizontalGalLength; // Initialize Locomotive Scroll
...
lscroll.on('scroll', _ScrollTrigger.ScrollTrigger.update); // If horizontal scroll is enabled, update the length of the scroll

_ScrollTrigger.ScrollTrigger.scrollerProxy(pageContainer, {
...
Promise.all([(0, _utils.preloadImages)('.gallery__item-imginner'), (0, _utils.preloadFonts)('vxy2fer')]).then(function () {
  ...
  var pinWrap = document.querySelector('.gallery');
  var pinWrapWidth = pinWrap.offsetWidth;
  var horizontalScrollLength = pinWrapWidth - window.innerWidth; // Pinning and horizontal scrolling

  horizontalGalLength = horizontalScrollLength; // Trigger that makes the gallery scroll horizontally

  _gsap.gsap.to('.gallery', {
    scrollTrigger: {
  ...
  _ScrollTrigger.ScrollTrigger.addEventListener('refresh', function () {
    return lscroll.update();
  }); //locomotive-scroll


  _ScrollTrigger.ScrollTrigger.refresh(); // Add Clones for background


  var mql = window.matchMedia('(min-width: 999px)'); // Desktop only (blurred background causes lag on mobile)

  if (mql.matches) {
    allImages.forEach(function (img) {
      if (img.parentNode.classList.value !== 'cover-img') {
        // skip 2 images
        if (img.style.backgroundImage === 'url("/pexels-loc-dang-8433972.101d1a23.jpg")' || img.style.backgroundImage === 'url("/pexels-loc-dang-1648023.45153c83.jpg")') return;
        ...
        if (img.style.backgroundImage === 'url("/pexels-loc-dang-2421375.17b2c728.jpg")') {
        ...
        var vgal = img.parentNode.parentNode.parentNode.parentNode.childNodes;
        vgal.forEach(function (node) {
...
},{"./utils":"MgTz","./cursor":"LMRJ","locomotive-scroll":"ez7q","gsap":"TpQl","gsap/ScrollTrigger":"TgBN"}]},{},["QvaY"], null)